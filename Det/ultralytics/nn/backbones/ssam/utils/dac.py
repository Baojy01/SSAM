...
"""
Deformable Axial Convolution. This code references to:
Repo: https://github.com/YaoleiQi/DSCNet/blob/main/DSCNet_2D_opensource/Code/DRIVE/S3_DSConv_pro.py
Paper: https://arxiv.org/pdf/2307.08388v2
"""


class DAConv(nn.Module):
    def __init__(self, in_channels: int, out_channels: int, kernel_size: int = 9, groups: int = 1,
                 extend_scope: float = 1.0, morph: int = 0, if_offset: bool = True):
        """
        Args:
        ...
        if morph == 0:  # for x-axial
            self.kernel_size = (kernel_size, 1)
            self.stride = (kernel_size, 1)
        elif morph == 1:  # for y-axial
            self.kernel_size = (1, kernel_size)
            self.stride = (1, kernel_size)
        else:
            raise ValueError("morph must be 0 or 1.")

        self.gn_offset = nn.GroupNorm(kernel_size, kernel_size)
        self.tanh = nn.Tanh()

        self.offset_conv = nn.Sequential(nn.Conv2d(in_channels, in_channels, kernel_size=3, padding=1, groups=in_channels, bias=False),
                                         nn.Conv2d(in_channels, kernel_size, 1, bias=False))

        self.dac_conv = nn.Conv2d(in_channels, out_channels, self.kernel_size, self.stride, groups=groups, bias=False)
    ...
    def forward(self, x):
        ...
        # Perform deformable conv
        coordinate_maps = get_coordinate_map_2D(offset=offset, morph=self.morph, extend_scope=self.extend_scope, device=x.device)
        deformed_feature = get_interpolated_feature(x, coordinate_maps[0], coordinate_maps[1])

        out = self.dac_conv(deformed_feature)
...
def get_coordinate_map_2D(
    ...
    """Computing 2D coordinate map
    Args:
        offset: offset predict by network with shape [B, K, W, H]. Here K refers to kernel size.
        morph: the morphology of the convolution kernel is mainly divided into two types along the x-axis (0) and the y-axis (1) (see the paper for details).
        extend_scope: the range to expand. Defaults to 1 for this method.
    ...
    batch_size, kernel_size, width, height = offset.shape
    ...
    if morph == 0:
        ...
        y_offset_ = einops.rearrange(offset, "b k w h -> k b w h")
        ...
        x_offset_ = einops.rearrange(offset, "b k w h -> k b w h")
...
def get_interpolated_feature(
    ...
    """From coordinate map interpolate feature.
...
def _coordinate_map_scaling(
    ...
    """Map the value of coordinate_map from origin=[min, max] to target=[a,b].
    ...
    coordinate_map_scaled = torch.clamp(coordinate_map, min_, max_)

    scale_factor = (b - a) / (max_ - min_)
    coordinate_map_scaled = a + scale_factor * (coordinate_map_scaled - min_)
